...
/********************************************************************/
static int cm36686_dump_reg(struct i2c_client *client);
int cm36686_enable_ps(struct i2c_client *client, int enable)
{
	...
	return 0;
ENABLE_PS_EXIT_ERR:
	APS_ERR("Enable ps function err\n");
	cm36686_dump_reg(client);
	return res;
}
...
int cm36686_setup_eint(struct i2c_client *client)
{
	...
	if (cm36686_obj->irq_node) {
		...
		if (request_irq
		    (cm36686_obj->irq, cm36686_eint_handler, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
...
static int set_psensor_threshold(struct i2c_client *client)
{
	...
	databuf[0] = CM36686_REG_PS_THD;
	databuf[1] = atomic_read(&obj->ps_thd_val_low);
	databuf[2] = atomic_read(&obj->ps_thd_val_high);
...
static int cm36686_init_client(struct i2c_client *client)
{
	...
	databuf[1] = 0x10;
	databuf[2] = 0x30;//need to confirm interrupt mode PS_MS mode whether to set 0x00-->0x30
	res = CM36686_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);
	if (res <= 0) {
		APS_ERR("i2c_master_send function err\n");
		goto EXIT_ERR;
...
static int ps_get_data(int *value, int *status)
{
	...
	APS_LOG("cm36686_obj ps data value = %d\n", value);
...
static int cm36686_dump_reg(struct i2c_client *client)
{
	...
	databuf[0] = 0x00;
	APS_LOG("*******************Start***************************************");
	APS_LOG("REG:%x", databuf[0]);
	CM36686_i2c_master_operate(client, databuf, 0x201, I2C_FLAG_READ);
	APS_LOG("value: value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
	databuf[0] = 0x02;
	APS_LOG("REG:%x", databuf[0]);
	CM36686_i2c_master_operate(client, databuf, 0x201, I2C_FLAG_READ);
	APS_LOG("value: value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
