...
static struct cm36283_priv *g_cm36283_ptr;
...
static int update_alsps_data(void)
{
	...
	cm36283_get_cust_alsps_hw()->i2c_addr[0]  = sensors_tuning_data->cm36283_alsps_i2c_addr[0];
	cm36283_get_cust_alsps_hw()->i2c_num   = sensors_tuning_data->cm36283_alsps_i2c_num;
	cm36283_get_cust_alsps_hw()->ps_threshold_high    = sensors_tuning_data->cm36283_ps_threshold_high;
	cm36283_get_cust_alsps_hw()->ps_threshold_low    = sensors_tuning_data->cm36283_ps_threshold_low;
	/* APS_LOG(" update alsps: i2c_addr=0x%x,i2c_num=%d,ps_threshold_high=%d,ps_threshold_low\n",cm36283_get_cust_alsps_hw()->i2c_addr[0],cm36283_get_cust_alsps_hw()->i2c_num,cm36283_get_cust_alsps_hw()->ps_threshold_high,cm36283_get_cust_alsps_hw()->ps_threshold_low); */
        for (i = 0; i < 15; i++)
	    cm36283_get_cust_alsps_hw()->als_level[i] = sensors_tuning_data->cm36283_als_level[i];
        for (i = 0; i < 16; i++)
	    cm36283_get_cust_alsps_hw()->als_value[i] = sensors_tuning_data->cm36283_als_value[i];
    }
    else
        APS_ERR("[%s]: get tuning data fail\n", __func__);
    return 0;
}
