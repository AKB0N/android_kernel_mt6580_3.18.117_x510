...
extern u8 sensor_id;
extern int sensor_id_g;
extern u16 ic_fw_version_g;
extern char ic_g[8];
extern char module_name_g[MODULE_MAX_LEN];
extern int fix_tp_proc_info(void *tp_data,u8 data_len);
extern void get_module_name(int sensor_id,char *module_name);
...
static u8 gup_burn_fw_app_code(struct i2c_client *client)
{
    ...
    u8  retry = 0;
    s32 ret = 0;
    u16 start_index = 4*FW_SECTION_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH + FW_BOOT_ISP_LENGTH + FW_LINK_LENGTH; // 32 + 4 + 2 + 4 = 42K
    //u16 start_index;
...
file_fail:  
	...
	len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x",ic_g,module_name_g,i2c_client_point->addr,ic_fw_version_g,RGK_SAMPLE_VERSION);
	#else
	len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x",ic_g,module_name_g,i2c_client_point->addr,ic_fw_version_g);
	#endif
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    {
        GTP_ERROR("Inconsistent firmware size. File size: %d, default fw size: %ld", update_msg.fw_total_len, sizeof(gtp_hotknot_calibration_section0));
        set_fs(update_msg.old_fs);
        _CLOSE_FILE(update_msg.file);
        return FAIL;
    }
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head *fw_head, char *path)
{
    ...
    {
    	update_msg.fw_total_len = sizeof(gtp_hotknot_calibration_section0);
