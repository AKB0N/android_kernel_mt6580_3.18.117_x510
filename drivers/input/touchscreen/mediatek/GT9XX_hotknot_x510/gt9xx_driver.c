...
unsigned char CUST_EINT_TOUCH_PANEL_NUM;
unsigned char EINTF_TRIGGER_RISING;
unsigned char EINTF_TRIGGER_FALLING;
...
//#ifdef VELOCITY_CUSTOM
	...
	len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x",ic_g,module_name_g,client->addr,ic_fw_version_g,RGK_SAMPLE_VERSION);
	#else
	len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x",ic_g,module_name_g,client->addr,ic_fw_version_g);
	#endif
