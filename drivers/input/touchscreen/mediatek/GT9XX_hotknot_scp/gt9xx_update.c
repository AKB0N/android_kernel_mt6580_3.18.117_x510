 ...
 *          2. add CONFIG_GTP_HEADER_FW_UPDATE switch to update fw by gtp_default_fw in *.h directly
 *                  by Meta, 2013/04/18
 *      V2.0:
...
#define FIRMWARE_LENGTH            \
	(FW_SS51_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH + FW_DSP_ISP_LENGTH + FW_BOOT_ISP_LENGTH + FW_APP_CODE_LENGTH)
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
	...
	if (update_msg.fw_total_len != fw_len) {
		GTP_ERROR("Inconsistent firmware size, Update aborted! Default size: %d(%dK), actual size: %d(%dK)",
	...
	if (update_msg.force_update != 0xBE) {
		GTP_INFO("FW checksum error,need enter update.");
		return SUCCESS;
	}
...
static void gup_search_file(s32 search_type)
{
	...
	for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
		...
		if (search_type & AUTO_SEARCH_BIN) {
			GTP_DEBUG("Search for %s, %s for fw update.(%d/%d)", UPDATE_FILE_PATH_1, UPDATE_FILE_PATH_2,
		...
		if ((search_type & AUTO_SEARCH_CFG) && !(got_file_flag & CFG_FILE_READY)) {
			GTP_DEBUG("Search for %s, %s for config update.(%d/%d)", CONFIG_FILE_PATH_1, CONFIG_FILE_PATH_2,
				  i + 1, GUP_SEARCH_FILE_TIMES);
...
static u8 gup_recall_check(struct i2c_client *client, u8 *check_src, u16 start_rd_addr, u16 check_length)
{
	u8 rd_buf[PACK_SIZE + GTP_ADDR_LENGTH];
	s32 ret = 0;
	...
	while (recall_length < check_length) {
		frame_length =
			((check_length - recall_length) > PACK_SIZE) ? PACK_SIZE : (check_length - recall_length);
		...
		if (memcmp(&rd_buf[GTP_ADDR_LENGTH], &check_src[recall_length], frame_length)) {
			GTP_ERROR("Recall frame data fail,not equal.");
			GTP_DEBUG("check_src array:");
			GTP_DEBUG_ARRAY(&check_src[recall_length], frame_length);
	...
	GTP_DEBUG("Recall check %dk firmware success.", (check_length / 1024));
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	ret =
	    gup_load_section_file(fw_boot_isp, (update_msg.fw_burned_len - FW_DSP_ISP_LENGTH), FW_BOOT_ISP_LENGTH,
				  SEEK_SET);
	if (FAIL == ret) {
...
s32 gup_update_proc(void *dir)
{
	...
	ret = gup_check_update_file(i2c_client_point, &fw_head, (u8 *) dir);
	...
	GTP_INFO("917S firmware detect add function temp_data = (%c, %c, %c, %c)!", temp_data[0], temp_data[1],
...
u8 gup_check_fs_mounted(char *path_name)
{
	...
	if (path.mnt->mnt_sb == root_path.mnt->mnt_sb) {/*-- not mounted*/
		path_put(&root_path);
		path_put(&path);
		return FAIL;
	}
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
	...
	ret = update_msg.file->f_op->read(update_msg.file,
					  (char *)gtp_default_FW_fl, update_msg.fw_total_len, &update_msg.file->f_pos);
	if (ret < 0) {
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_default_FW_fl[FW_HEAD_LENGTH], FW_DOWNLOAD_LENGTH);
	...
	if (GTP_FL_FW_BURN == dwn_mode) {
		ret = gup_check_and_repair(i2c_client_point,
					   0xC000,
					   &gtp_default_FW_fl[FW_HEAD_LENGTH + FW_DOWNLOAD_LENGTH], FW_DOWNLOAD_LENGTH);
...
#endif
static u8 gup_check_and_repair(struct i2c_client *client, s32 check_start_addr, u8 *target_fw, u32 check_total_length)
{
	s32 ret = 0;
	u32 checked_len = 0;
	u8 checked_times = 0;
	u32 check_addr = 0;
	u8 check_buf[GUP_FW_CHK_SIZE];
	u32 rd_size = 0;
	u8 flag_err = 0;
	s32 i = 0;

	check_addr = check_start_addr;
	while ((checked_times < MAX_CHECK_TIMES) && (checked_len < check_total_length)) {
		rd_size = check_total_length - checked_len;
		if (rd_size >= GUP_FW_CHK_SIZE)
			rd_size = GUP_FW_CHK_SIZE;
#if GTP_SUPPORT_I2C_DMA
		ret = i2c_read_bytes(client, check_addr, check_buf, rd_size);
#else
		if (!i)
			ret = i2c_read_bytes(client, check_addr, check_buf, rd_size);
		else
			ret = i2c_auto_read(client, check_buf, rd_size);
#endif

		if (-1 == ret) {
			GTP_ERROR("Read check ram fw i2c error");
			checked_times++;
			continue;
		}

		for (i = 0; i < rd_size; i++) {
			if (check_buf[i] != target_fw[i]) {
				GTP_ERROR("check_buf[%d] = 0x%x, target_fw[%d] = 0x%x.",
					i, check_buf[i], i, target_fw[i]);
				GTP_ERROR("Ram pos[0x%04x] checked failed,rewrite.", check_addr + i);
				i2c_write_bytes(client, check_addr + i, &target_fw[i], rd_size - i);
				flag_err = 1;
				i = 0;
				break;
			}
		...
		if (!flag_err) {
			GTP_DEBUG("Ram pos[0x%04X] check pass!", check_addr);
			checked_len += rd_size;
			target_fw += rd_size;
			check_addr += rd_size;
		} else {
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client,
			      0xC000, &gtp_default_FW_fl[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH], FW_DSP_LENGTH);
	if (ret == -1) {
	...
	if (GTP_FL_FW_BURN == dwn_mode) {
		ret = gup_check_and_repair(client,
					   0xC000,
					   &gtp_default_FW_fl[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH], FW_DSP_LENGTH);
...
s32 gup_load_fx_system(void)
{
	...
	length = firmware[0] * 256 * 256 * 256 + firmware[1] * 256 * 256 + firmware[2] * 256 + firmware[3];
